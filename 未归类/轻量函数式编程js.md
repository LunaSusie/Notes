...
* 函数式编程中实现稍后传参有两种方式。偏函数和柯里化，它们都是通过闭包实现，返回的是一个，对原来函数包装过的函数。
* 偏函数，对原来函数进行一个包装，使用闭包返回一个指定第一个参数，第二个参数.....的函数，就是已经被指定了第一个参数，第二个参数......，只要输入剩余参数即可运行。
* 翻转函数的参数。
* 从右边指定函数的参数，而不影响调用偏函数的时候参数的顺序。先把原来函数反转参数，构建偏函数，在再把参数顺序反转回来。
* 
* 
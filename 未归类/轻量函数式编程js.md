...
* 偏函数，减少函数参数个数的过程。
* 实现：输入函数名称，和数个该函数的实参（顺序左到右），返回一个指定第一个参数，第二个参数.....的函数。在指定最后一个参数之前的所有返回都只是一个中间过程，只有指定了最后一个参数，函数才会执行。
* 反转函数的参数，返回一个形参被翻转的函数。
* 右偏函数，返回一个从右边开始指定顺序的偏函数，而不影响偏函数调用的时候实参的顺序：先把原来函数参数反转，构建偏函数，在再把参数顺序反转回来。

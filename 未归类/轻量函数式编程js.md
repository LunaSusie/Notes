...
* 别用this。
## 管理输入（输入参数）
* 一个函数如果有多个形参，一次性传入所有实参，还是一次传递一个参数。
...
* 反柯里化，柯里化的函数可以可以还原成和柯里化前相识的函数，只是相识函数，和原函数不是完全相同的。
* 只接收一个实参的函数。
* 接收一个实参，并返回的函数。
...
* 无参风格。通过移除不必要的形参-实参映射来减少视觉上的干扰。
### 总结 
* 函数的输入管理，就是对一个函数参数改造，指定/合并/分解函数的某几个参数，然后返回一个新函数
	* 偏函数，返回一个指定了函数某几个（不是全部，全部就没有意义了）参数值，
	* 返回只接收原函数第一个参数的函数。
	* 返回接收一个反转原函数参数顺序的函数。
